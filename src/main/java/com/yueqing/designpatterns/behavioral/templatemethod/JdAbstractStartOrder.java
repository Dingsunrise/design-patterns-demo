 ...
 * @date 2022/7/20
 */
public class JdAbstractStartOrder extends AbstractStartOrderTemplate {

    @Override
    protected double calculatePrice(double price) {
        return PriceStrategyFactory.getPriceStrategy(OrderPriceEnum.JdOrder).calculatePrice(price);
    }

    @Override
    protected void ship(Commodity commodity) {
        System.out.println("The "+commodity.getName()+" has been shipped by Jingdong Express");
    }
}
